...
* -Algorimo de multiplicación Filas X Filas
*************************************************************************/
...
for (i = 0; i < SZ; i++) {
    for (j = 0; j < SZ; j++) {
        double *pA, *pB, S;
        S = 0.0;
        pA = a + (i * SZ); // Fila i de la matriz A
        pB = b + (j * SZ); // Fila j de la matriz B

        for (k = 0; k < SZ; k++) {
            S += (*pA++ * *pB++ *-1); // Multiplicación de elementos correspondientes de las filas
        }
